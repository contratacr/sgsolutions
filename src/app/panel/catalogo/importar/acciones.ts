...
import {ErrorImportacion,prepararArchivo} from '@/lib/intcomex/archivo';
...
export async function importarArchivo(form:FormData,revisionEsperada?:number){
 try{
  ...
  if(moneda!=='USD'&&moneda!=='CRC')throw new ErrorImportacion('moneda');
  const filas=await readSheet(Buffer.from(await archivo.arrayBuffer()));
  const r=prepararArchivo(filas,base.catalogo,moneda,String(categoria??''),new Date().toISOString());
  if(revisionEsperada!==undefined){
   ...
   for(const p of r.catalogo.productos){
  ...
  return {preview:{revision:base.revision,informe:r.informe,cambios:r.cambios}};
