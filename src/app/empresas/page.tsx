...
export default async function Empresas() {
  const t = await getTranslations('Empresas'); const n = await getTranslations('Navegacion');
  return <main id="contenido" className="empresas-editorial"><section className="empresa-portada"><div className="empresa-portada-imagen"><Image src="/imagenes/red-hero.webp" alt={t('foto')} fill priority sizes="(max-width: 760px) 100vw, 65vw"/></div><div className="contenedor empresa-portada-contenido"><Link className="empresa-volver" href="/"><ArrowLeft size={14}/>{n('volver')}</Link><p className="etiqueta etiqueta-clara">{t('etiqueta')}</p><h1>{t('titulo')}</h1><p className="empresa-descripcion">{t('descripcion')}</p><a className="boton boton-naranja" href={enlaceWhatsApp(t('mensaje'))} target="_blank" rel="noopener noreferrer">{t('accion')}<ArrowUpRight size={18}/></a></div></section><nav className="empresa-necesidades contenedor" aria-label={t('necesidades')}>{[1,2,3].map(i => <a key={i} href={`#solucion-${i}`}><span>{`0${i}`}</span><div><strong>{t(`necesidad${i}`)}</strong><p>{t(`necesidad${i}Texto`)}</p></div><ArrowRight size={19}/></a>)}</nav><Soluciones/><Proyectos/><Contacto/></main>;
}
