...
export function Cabecera() {
  ...
  </div>{abierto && <nav id="navegacion-movil" aria-label={t('principal')} className="nav-movil">{enlaces.map(([href, texto]) => <Link onClick={() => setAbierto(false)} href={href} key={texto} aria-current={ruta === href ? 'page' : undefined}>{t(texto)}</Link>)}<Link href="/contacto" onClick={() => setAbierto(false)}>{t('contacto')}</Link></nav>}</header>;
}
