...
export async function ServiciosSoporte(){
 ...
 return <section className="contenedor seccion servicios-soporte">
  ...
  <div className="servicios-grupos">{[8,5,5,4,3].map((cantidad,i)=><details className="servicio-grupo" key={i}>
   ...
   <ul>{Array.from({length:cantidad},(_,j)=><li key={j}><div><h4>{t(`g${i}s${j}Titulo`)}</h4><p>{t(`g${i}s${j}Texto`)}</p></div><a href={enlaceWhatsApp(t('mensaje',{servicio:t(`g${i}s${j}Titulo`)}))} target="_blank" rel="noopener noreferrer" aria-label={`${t('consultar')}: ${t(`g${i}s${j}Titulo`)} · ${t('nueva')}`}>{t('consultar')}<ArrowUpRight size={16}/></a></li>)}</ul>
  </details>)}</div>
