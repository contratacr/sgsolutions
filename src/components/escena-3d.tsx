...
export default function Escena3D({activo, punteroRef, invalidarRef, alCargar, alFallar, reducir=false}: Props) {
  const elemento = useRef<HTMLDivElement>(null);
  const seleccion = useRef(activo);
  const movimientoReducido=useRef(reducir);
  useEffect(()=>{movimientoReducido.current=reducir;},[reducir]);
  ...
  useEffect(() => {
    ...
    function dibujar(tiempo: number) {
      ...
      if(movimientoReducido.current && listo)return;
      const delta = Math.min((tiempo-anterior)/1000,.08);
      anterior = tiempo;
      if(!movimientoReducido.current)transcurrido += delta;
    ...
    return () => {
      destruido = true; renderer.setAnimationLoop(null); observador.disconnect(); invalidarRef.current = () => {};
