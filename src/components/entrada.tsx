...
const destinos = [
  ...
  { clave: 'soporte', href: '#contacto', Icono: Headset },
  { clave: 'empresas', href: '#servicios', Icono: Network },
] as const;
...
export function Entrada() {
  ...
  return <LazyMotion features={cargarAnimaciones} strict><MotionConfig reducedMotion="user">
    <section className="entrada" aria-labelledby="entrada-titulo">
        ...
        <div className="entrada-destinos">
          ...
          <nav aria-label={t('menu')} className="entrada-menu" onMouseLeave={() => setActivo(0)}>
            {destinos.map(({clave, href, Icono}, indice) => <m.div key={clave} whileHover={{y: -3}} whileTap={{scale: .99}} transition={{duration: .2}}>
              <Link href={href} className={`destino destino-${clave}`} onPointerEnter={() => setActivo(indice)} onFocus={() => setActivo(indice)}>
                ...
                <span className="destino-accion">{t(`${clave}Accion`)}<span aria-hidden="true"> →</span></span>
              </Link>
            </m.div>)}
          </nav>
      ...
      <div className="contenedor entrada-base"><a href="#servicios">{t('explorar')}<ArrowDown size={16}/></a><span><MapPin size={14}/>{t('ubicacion')}</span></div>
    </section>
