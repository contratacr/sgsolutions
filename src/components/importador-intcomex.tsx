...
export function ImportadorIntcomex({categorias,fecha}:{categorias:CatalogoAdmin['categorias'];fecha?:string}){
 ...
 function ejecutar(aplicar=false){iniciar(async()=>{
  if(aplicar)busqueda.current++;setEstado('');try{const form=new FormData();if(archivo)form.set('archivo',archivo);form.set('moneda',moneda);form.set('categoria',categoria);form.set('fotos',JSON.stringify(Object.fromEntries(Object.entries(fotos).map(([sku,f])=>[sku,f.src]))));
  ...
  else if(r.preview){setPreview(r.preview);setPagina(0);void buscarFotos(r.preview);}
  else{setPreview(undefined);setEstado('guardado');router.refresh();}
  }catch{setEstado('error');}
 });}
 return <section className="admin-editor admin-importacion">
  <p>{t('descripcion')}</p><p>{fecha?t('ultima',{fecha:new Intl.DateTimeFormat(idioma==='es'?'es-CR':'en-US',{dateStyle:'medium',timeStyle:'short',timeZone:'America/Costa_Rica'}).format(new Date(fecha))}):t('sinFecha')}</p>
  ...
  <fieldset disabled={ocupado} className="admin-importar-campos"><legend>{t('seleccionar')}</legend>
   ...
   <label>{t('monedaLabel')}<select value={moneda} onChange={e=>{setMoneda(e.target.value);limpiar();}}><option value="">{t('elegirMoneda')}</option><option value="USD">{t('usd')}</option><option value="CRC">{t('crc')}</option></select></label>
   ...
   <button type="button" className="boton boton-azul" disabled={!archivo||!moneda} onClick={()=>ejecutar()}>{t(ocupado?'procesando':'previsualizar')}</button>
  </fieldset>
  <p role="status">{estado&&t(estado,{fila})}</p>
  {preview&&<section aria-label={t('vistaPrevia')}>
   <h2>{t('vistaPrevia')}</h2><p role="status">{t(progreso.hechas<progreso.total?'buscandoFotos':'fotosListas',{hechas:progreso.hechas,total:progreso.total,encontradas:Object.keys(fotos).length})}</p><p>{t('fotosNota')}</p><p>{t('resumen',{...preview.informe})}</p><p>{t('reglas')}</p>
   <div className="admin-importar-tabla"><table><caption>{t('detalle')}</caption><thead><tr>{['producto','fotoLabel','costo','stockLabel','venta'].map(k=><th key={k} scope="col">{t(k)}</th>)}</tr></thead><tbody>{preview.cambios.slice(pagina*20,(pagina+1)*20).map(c=><tr key={c.sku}><td><strong>{c.sku}</strong><br/>{c.nombre}{c.nuevo&&<p>{t('borrador')}</p>}</td><td>{fotos[c.sku]?<><Image src={fotos[c.sku].src} alt={c.nombre} width={100} height={100} unoptimized style={{objectFit:'contain'}}/><a href={fotos[c.sku].fuente} target="_blank" rel="noopener noreferrer" aria-label={t('fuenteAccesible')}>{t('fuente')}</a><button type="button" disabled={ocupado} onClick={()=>setFotos(prev=>{const copia={...prev};delete copia[c.sku];return copia;})}>{t('descartarFoto')}</button></>:t(c.sinFoto?'fotoPendiente':'fotoConservada')}</td><td>{c.costoAntes!==null&&<span>{c.costoAntes} {c.monedaAntes} → </span>}{c.costo} {moneda}</td><td>{c.stock===null?t('consultar'):c.exacto?c.stock:t('alMenos',{cantidad:c.stock})}</td><td>{c.precio===null?t('revision'):colones(c.precio)}{c.manual&&<p>{t('manual')}</p>}{c.revision&&<p>{t('alertaPrecio')}</p>}</td></tr>)}</tbody></table></div>
