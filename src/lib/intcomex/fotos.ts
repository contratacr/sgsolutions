...
export async function buscarFotoIntcomex(sku:string,mpn:string){
 ...
 for(const termino of [...new Set([sku,mpn].filter(Boolean))]){
  try{
   ...
   const detalle=(await descargar(fuente,1500000,signal)).toString('utf8');
   const tag=detalle.match(/<img[^>]*ws_images_style[^>]*>/)?.[0];
   const src=tag?.match(/src=["']([^"']+)["']/)?.[1]?.replace(/&amp;/g,'&');
   const urls=[src,`${origen}/images/products/${encodeURIComponent(producto.sku)}%20M.jpg`,`${origen}/images/products/${encodeURIComponent(producto.sku)}%20M.png`];
   for(const posible of urls){
    ...
    try{const bytes=await descargar(url,4*1024*1024,signal);const m=imageSize(bytes);
     if(!m.width||!m.height||Math.min(m.width,m.height)<160||m.width*m.height>40000000||!['jpg','png','webp'].includes(m.type??''))continue;
