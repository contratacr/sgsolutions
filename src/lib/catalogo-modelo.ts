...
export function publicarCatalogo(c:CatalogoAdmin){return {categorias:c.categorias,textos:c.textos,productos:c.productos.filter(p=>p.publicado).map(p=>({id:p.id,categoria:p.categoria,nombre:p.nombre,descripcion:p.descripcion,imagen:p.imagen,marca:p.marca,codigoFabricante:p.codigoFabricante,destacado:p.destacado,disponibilidad:p.proveedor?(p.proveedor.tipo==='Downloadable'||p.proveedor.tipo==='License'?'consultar':p.proveedor.stock===null?'consultar':p.proveedor.stock===0?'agotado':'proveedor'):p.disponibilidadReferencia,actualizado:p.proveedor?.actualizado??null,precio:p.revisionPrecio&&p.precioManual===null?null:(calcularPrecio(p.costoUsd,c.ajustes,p.precioManual,p.costoCrc)??p.precioReferencia)}))};}
